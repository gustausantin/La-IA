-- =====================================================
-- MIGRACIÓN: CORREGIR ERRORES EN generate_availability_slots_employee_based
-- Fecha: 2025-11-15
-- Objetivo: 
--   1. Corregir referencias incorrectas a columna is_active
--      que no existe en employee_schedules y employee_absences
--   2. Corregir ON CONFLICT que usa restricción única inexistente
--      cambiando a verificación IF NOT EXISTS
-- =====================================================

-- Corregir la función generate_availability_slots_employee_based
-- para usar las columnas correctas:
-- - employee_schedules.is_working (en lugar de is_active)
-- - employee_absences.start_date/end_date (en lugar de absence_date e is_active)
...
BEGIN
    ...
    LOOP
        ...
        WHILE v_current_date <= v_end_date LOOP
            ...
            AND is_working = true  -- ✅ CORREGIDO: usar is_working en lugar de is_active
            LIMIT 1;
            ...
            IF v_schedule IS NOT NULL THEN
                ...
                SELECT EXISTS(
                    ...
                    WHERE employee_id = v_employee.id
                    AND v_current_date >= start_date  -- ✅ CORREGIDO: usar start_date/end_date en lugar de absence_date
                    AND v_current_date <= end_date
                    -- ✅ CORREGIDO: eliminar condición is_active que no existe
                ) INTO v_has_absence;
                
                -- Solo generar si NO hay ausencia
                IF NOT v_has_absence THEN
                    -- Obtener recurso asignado (mesa)
                    v_resource_id := v_employee.assigned_resource_id;  -- ✅ CORREGIDO: eliminar referencia a v_schedule.resource_id que no existe
...
DO $$
BEGIN
    RAISE NOTICE '✅ Función generate_availability_slots_employee_based corregida:';
    RAISE NOTICE '   - is_active -> is_working en employee_schedules';
    RAISE NOTICE '   - Consulta de ausencias corregida (start_date/end_date)';
    RAISE NOTICE '   - ON CONFLICT reemplazado por IF NOT EXISTS';
END $$;

SELECT 'Migración 20251115_06_fix_is_active_column_error completada' AS status;
